...
export class UserAPI {
    ...
    this.router.put('/:id', bparser.json(), (req: Request, res: Response) => {
      UserModel.findById(req.params.id, (err, user) => {
        if( err ) this.errorHandler(err);
          ...
          if( req.body ) {
            this.modifyUser( user, <User>req.body, ( resp: User ) => { 
  ...
  private static modifyUser( dbUser: UserDocument, data: User, cb: Function ): void {
    if( dbUser && data ){
  ...
  private static errorHandler(error: any, response?: Response): void {
